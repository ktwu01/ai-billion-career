...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "image/*", "text/plain", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for user-documents" ON storage.objects FOR SELECT USING (bucket_id = 'user-documents');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for user-documents" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'user-documents');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for user-documents" ON storage.objects FOR UPDATE USING (bucket_id = 'user-documents');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for user-documents" ON storage.objects FOR DELETE USING (bucket_id = 'user-documents');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'user-documents',
        public: true,
        allowed_mime_types: ["application/pdf", "image/*", "text/plain", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"],
